...
function Header()
{
	...
	$this->Text(60,35,'REPORTE DE PRODUCTORES POR DISTRITO',0,'C', 0);
	$this->Ln(10);
}
    ...
    $pdf->SetTextColor(255);
		...
		for($i=0;$i<1;$i++)
			{
				$pdf->Row(array('DISTRITO', 'NO. ACUACULTORES'));
			}
	
	$historial = $con->conectar();	
	$strConsulta = "SELECT d.distrito,d.idacuicultor,COUNT(*) as contador
	...
	for ($i=0; $i<$numfilas; $i++)
		{
			...
			if($i%2 == 1)
			{
    			...
    			$pdf->SetTextColor(0);
				$pdf->Row(array($fila['distrito'], $fila['contador']));
			}
			else
			{
			$pdf->SetFillColor(255,255,255);
